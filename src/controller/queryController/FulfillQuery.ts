import CourseSection from "../datasetController/CourseSection";
import ApplyController from "./ApplyController";

export default class FulfillQuery {

    public listOfColumns: string[] = [];
    public applyKeys: string[] = [];
    public applyKeysCategories: string[] = [];
    public applyKeyNames: string[] = [];
    public groupKeys: string[] = [];

    public fulfillQuery(query: any, section: any, datasetName: string) {
    ...
    public fulfillApply(query: any) {
        ...
        for (let key of group) {
            let temp = key.slice(key.lastIndexOf("_") + 1, key.length);
            this.groupKeys.push(temp);
        }
    }

    public fulfillOptions(query: any) {
        this.listOfColumns = [];
        ...
        for (let key of this.applyKeys) {
            if (!this.listOfColumns.includes(key)) {
                this.listOfColumns.push(key);
            }
        }
    ...
    public fulfillWhere(query: any, section: any): boolean {
    ...
    public fulfillGT(gtClause: any, section: any): boolean {
    ...
    public fulfillLT(ltClause: any, section: any): boolean {
    ...
    public fulfillEQ(eqClause: any, section: any): boolean {
    ...
    public fulfillIS(isClause: any, section: any): boolean {
    ...
    public fulfillAND(andClause: any, section: any): boolean {
    ...
    public fulfillOR(orClause: any, section: any): boolean {
    ...
    public fulfillNOT(notClause: any, section: any): boolean {
        return !this.fulfillWhere(notClause, section);
    }

    public fulfillTransformations(query: any, result: any, columnsArray: any): any[] {
        return this.fulfillGroup(query, result, columnsArray);
    }

    public fulfillGroup(query: any, result: any, columnsArray: any): any {
        let listOfGrouping: any[] = [];
        ...
        return this.fulfillApplyKeys(results, query, columnsArray);
    }

    public fulfillApplyKeys(listOfSectionGroup: any, query: any, columnsArray: any) {
        let result: any[] = [];
        for (let sectionGroup of listOfSectionGroup) {
            result.push(this.doApplyKeyAction(sectionGroup, query["GROUP"], columnsArray));
        }
        return result;
    }

    public doApplyKeyAction(sectionGroup: any, query: any, columnsArray: any) {
        let result = Object.create(null);
        for (let item of query) {
            if (columnsArray.includes(item)) {
        ...
        for (let i = 0; i < this.applyKeys.length; i++) {
            ...
            if (columnsArray.includes(keyName)) {
                if (this.applyKeysCategories[i] === "AVG") {
                    result[keyName] = ApplyController.averageFulfill(sectionGroup, fieldKey);
                } else if (this.applyKeysCategories[i] === "SUM") {
                    result[keyName] = ApplyController.sumFulfill(sectionGroup, fieldKey);
                } else if (this.applyKeysCategories[i] === "MAX") {
                    result[keyName] = ApplyController.maxFulfill(sectionGroup, fieldKey);
                } else if (this.applyKeysCategories[i] === "MIN") {
                    result[keyName] = ApplyController.minFulfill(sectionGroup, fieldKey);
                } else if (this.applyKeysCategories[i] === "COUNT") {
                    result[keyName] = ApplyController.countFulfill(sectionGroup, fieldKey);
                }
            }
        }
        return result;
    ...
    public orderResult(result: any[], queryOptions: any) {
    ...
    private sortHelper(result: any[], prevKeys: string[], key: any, dir: any) {
        result.sort(function (a, b) {
            let sort = true;
